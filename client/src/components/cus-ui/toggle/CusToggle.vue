...
const backgroundColor = computed(() => {
  ...
  return theme.primaryColor || cusVariables.cusColorPrimary;
});

const hoverBackgroundColor = computed(() => {
  const color = new CusColor(backgroundColor.value);
  return color.isDark()
    ? new CusColor(backgroundColor.value).lighten(0.1).hexColor
    : new CusColor(backgroundColor.value).darken(0.1).hexColor;
});

const foregroundColor = computed(
  () => new CusColor(backgroundColor.value).subColor(CusColor.white, 10, 7).hexColor
);
